...
CANONICAL_ITRANSFORMER_ENTRY = Path("baselines/PDT/run.py")
...
def build_command(repo_root: Path, manifest: Manifest, run_dir: Path) -> tuple[list[str], dict[str, str]]:
    ...
    if not isinstance(manifest.pred_len, int):
        raise TypeError("iTransformer builder expects a single pred_len after manifest expansion.")
    if manifest.repo_relative_entry != CANONICAL_ITRANSFORMER_ENTRY:
        raise ValueError(
            "iTransformer experiments must use the verified shared PDT model-zoo route "
            f"{CANONICAL_ITRANSFORMER_ENTRY.as_posix()}, got {manifest.repo_relative_entry.as_posix()}."
        )
    ...
    args.setdefault("model", "iTransformer")
    ...
    entry = repo_root / CANONICAL_ITRANSFORMER_ENTRY
