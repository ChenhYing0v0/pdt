...
class iterative_normalization_py(torch.autograd.Function):
    ...
    def forward(ctx, *args, **kwargs):
        ...
        if training:
            ...
            running_wmat.copy_(momentum * wm + (1. - momentum) * running_wmat)      # 更新running_wm
        else:
            xc = x - running_mean
            wm = running_wmat
        xn = wm.matmul(xc)                                                          # [g, nc, m] /hatX=Sigma^-1/2 * Xc
        Xn = xn.view(X.size(1), X.size(0), *X.size()[2:]).transpose(0, 1).contiguous()  # [N, C, H, W]
        ctx.save_for_backward(*saved)
        return Xn

    @staticmethod
    def backward(ctx, *grad_outputs):
        grad, = grad_outputs                                                        # gard_Xn, [N, C, H, W]
        ...
        grad_input = g_x.view(grad.size(1), grad.size(0), *grad.size()[2:]).transpose(0, 1).contiguous()    # 最终输入梯度，恢复为原始shape [N, C, H, W]
        return grad_input, None, None, None, None, None, None, None
...
class IterNorm(torch.nn.Module):
    def __init__(self, num_features, num_groups=1, num_channels=None, T=5, dim=4, eps=1e-5, momentum=0.1, affine=True,
        ...
        shape = [1] * dim
        shape[1] = self.num_features                            # shape为 [1, num_features, 1, 1] dim=4
        if self.affine:
