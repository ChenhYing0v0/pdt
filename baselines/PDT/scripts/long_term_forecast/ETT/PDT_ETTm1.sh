...
for ((i=0; i<${#PL_LIST[@]}; i++)); do
  ...
  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python -u "$BASELINE_DIR/run.py" \
    ...
    --seed "$SEED" \
    --des Exp_final_48 \
    --q_mat_file "dataset/RRR_mats/ETTm1/ETTm1_RRR_L48_R${R_RANK}_H${PL}_Qin.npy" \
    --q_out_mat_file "dataset/PCCA_mats/ETTm1/ETTm1_PCCA_OUT_H${PL}_identity_avg.npy" \
    --r_mat_file "dataset/RRR_mats/ETTm1/ETTm1_RRR_L48_R${R_RANK}_H${PL}_R.npy" \
    --rk_mat_file "dataset/RRR_mats/ETTm1/ETTm1_RRR_L48_R${K_TOP}_H${PL}_R.npy" \
    --r_rank "$R_RANK" \
    --k_top "$K_TOP" \
    --embed_size 16 \
    --alpha_init "$ALPHA" \
