...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _export_linear_encoder_A(self, output_dir):
        ...
        print("Exporting linear encoder A...")
        if hasattr(model, 'save_linear_encoder_A'):
            model.save_linear_encoder_A(output_dir)

    def _export_channel_mask(self, output_dir):
        model = self.model.module if hasattr(self.model, 'module') else self.model
        print("Exporting channel mask...")
        if hasattr(model, 'save_channel_mask'):
            model.save_channel_mask(output_dir)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                try:
                    model_for_alpha = self.model.module if hasattr(self.model, 'module') else self.model
                    if hasattr(model_for_alpha, 'get_alpha'):
                        alpha_val = model_for_alpha.get_alpha()
                        # scalar value for TB
                        self.writer.add_scalar(f'{self.pred_len}/train/alpha', alpha_val.item(), self.step)
                except Exception:
                    pass
        ...
        self._export_channel_mask(res_path)

        if self.args.save_cov:
            # 保存所有batch的init和协方差
            if hasattr(self.model, "save_all_inits_and_covs"):
                self.model.save_all_inits_and_covs()
    ...
    def test(self, setting, test=0):
        ...
        if self.args.save_cov:
            ...
            if hasattr(self.model, "save_all_inits_and_covs"):
                self.model.save_all_inits_and_covs(stage='test')
