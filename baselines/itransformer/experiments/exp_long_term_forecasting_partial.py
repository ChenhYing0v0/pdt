...
from utils.metrics import CORR, RSE, metric
...
class Exp_Long_Term_Forecast_Partial(Exp_Basic):
            ...
            for _, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        batch_size, tx, num_vars = batch_x.shape
                        _, ty, _ = dec_inp.shape
                        if batch_x_mark is None:
                            outputs = self.model(
                                batch_x.permute(0, 2, 1).reshape(batch_size * num_vars, tx, 1),
                                batch_x_mark,
                                dec_inp.permute(0, 2, 1).reshape(batch_size * num_vars, ty, 1),
                                batch_y_mark,
                            ).reshape(batch_size, num_vars, -1).permute(0, 2, 1)
                        else:
                            outputs = self.model(
                                batch_x.permute(0, 2, 1).reshape(batch_size * num_vars, tx, 1),
                                batch_x_mark.repeat(num_vars, 1, 1),
                                dec_inp.permute(0, 2, 1).reshape(batch_size * num_vars, ty, 1),
                                batch_y_mark.repeat(num_vars, 1, 1),
                            ).reshape(batch_size, num_vars, -1).permute(0, 2, 1)
                    else:
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.efficient_training:
                    _, _, num_vars = batch_x.shape
                    index = np.stack(random.sample(range(num_vars), num_vars))[-self.args.enc_in:]
    ...
    def test(self, setting, test=0):
        ...
        preds = []
        trues = []
        folder_path = os.path.join(self.args.test_results_dir, setting)
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 20 == 0:
                    ...
                    if test_data.scale and self.args.inverse:
                        ...
                        model_input = test_data.inverse_transform(model_input.squeeze(0)).reshape(shape)
        ...
        metrics_dir = os.path.join(self.args.results_dir, setting)
